...
local function preprocess_data(image_list, label_list, batch_size, image_size, mean, std)
	...
	label_tensor = torch.Tensor(batch_size,1):fill(0)
	...
	for i=1,1,batch_size do
		local raw_image = image.scale(image.load(image_nlist[i]),'^'..image_size)
		...
		label_tensor[i] = torch.Tensor({label_nlist[i]}):double()
	end
